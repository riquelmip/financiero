<?php 
	...
	class Configuracion extends Controllers{
		...
		public function Configuracion()
		{
			...
			$data['page_tag'] = "Configuracion";//Nombre superior
			$data['page_name'] = "Configuracion";//Nombre de la pagina 
			$data['page_title'] = "Configuracion"; //Nombre del titulo en la vista
			$data['page_functions_js'] = "functions_configuracion.js";// Funcion de js para las acciones
			$this->views->getView($this,"configuracion",$data);//Se refiere al nombre de la vista
		}
			...
			if ($_SESSION['permisosMod']['leer']) {
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if ($_SESSION['permisosMod']['actualizar']) {
						$btnEdit = '<button class="btn btn-primary btn-sm btnEditCargos" onClick="fntEditCargos('.$arrData[$i]['id_configuracion'].')" title="Editar"><i class="fas fa-pencil-alt"></i></button>';
					}
					//si tiene permiso de eliminar se agrega el boton
					if ($_SESSION['permisosMod']['eliminar']) {
						$btnDelete = '<button class="btn btn-danger btn-sm btnDelCargos" onClick="fntDelCargos('.$arrData[$i]['id_configuracion'].')" title="Eliminar"><i class="far fa-trash-alt"></i></button>';
					}
		...
		public function setCargos(){ //Hace la insercion y edita
			...
			$strCat =  strClean($_POST['txttiempo_incobrable']);
		...
		public function delCargos() //Elimina la categoria
		{
			if($_POST){
				if ($_SESSION['permisosMod']['eliminar']) {
					$intIdcatee = intval($_POST['id_configuracion']);
